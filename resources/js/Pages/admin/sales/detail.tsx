...
export default function SalesAddDashboard({
    ...
    const [countList, setCountList] = useState<number>(salesDetailList.length || 1);
    const [productList, setProductList] = useState<
    Array<{ master_id: number | null; stock_id: number | null; quantity: number | null; price: number | null }>
    >(
        salesDetailList.map((item: any) => {
            const stock = productInStock.find((p: any) => p.stock_id === item.stock_id);
            return {
            master_id: stock?.master_id || null,
    ...
    const { data, setData, reset, errors, processing, patch } = useForm<{
        ...
        product_list: salesDetailList.map((item: any) => ({
            stock_id: item.stock_id,
            quantity: item.quantity,
            price: item.price,
        })),
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        patch(route("sales.update", { id: data.id }), {
            onSuccess: () => reset(),
        });
    };
                    ...
                    <div className="grid w-full lg:max-w-lg items-center gap-2">
                        ...
                        {Array.from({ length: countList }).map((_, index) => (
                            <div key={index} className="mt-2">
                                <Label>Produk</Label>
                                <Select
                                    value={productList[index].stock_id?.toString() || ""}
